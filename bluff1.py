...
def predict_news(news_text):
    ...
    # model.predict may return 'TRUE' or something else per your training; preserve your mapping
...
st.markdown(
    ...
    .sidebar .block-container { padding-top: 8px; }
    .sidebar .stRadio > div { margin-top: 10px; }
    ...
    .stSidebar .stButton>button, .stSidebar .stMarkdown { color: #cfeeff; }
...
def page_history():
    ...
    if df.empty:
        ...
        st.dataframe(df.sort_values(by="timestamp", ascending=False).head(300), width='stretch')
    st.markdown("</div>", unsafe_allow_html=True)
